...
bool vb_push_free(uint order, uint startPage)
{
    ...
    for (;;)
    {
        ...
        vb_next[startPage] = old_idx == INVALID_HEAD_IDX ? INVALID_ID : old_idx;
...
bool ib_push_free(uint order, uint startPage)
{
    ...
    for (;;)
    {
        ...
        ib_next[startPage] = old_idx == INVALID_HEAD_IDX ? INVALID_ID : old_idx;
...
void main() {
    ...
    // }
    // uint starts[256];
    // for (uint dirtyIdx = 0; dirtyIdx < 256; dirtyIdx++)
    //     starts[dirtyIdx] = vb_alloc_pages(dirtyIdx % 8);
    
    // for (uint dirtyIdx = 0; dirtyIdx < 256; dirtyIdx++)
    //     vb_free_pages(starts[dirtyIdx], dirtyIdx % 8);

    // uint s0 = vb_alloc_pages(0);
    // uint s1 = vb_alloc_pages(1);

    // vb_free_pages(s0, 0);
    // vb_free_pages(s1, 1);

    // vb_alloc_pages(1);
    // uint v = vb_pop_free(1);
