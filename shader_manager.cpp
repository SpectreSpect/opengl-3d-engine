...
void ShaderManager::init_shaders(const std::string& root_path) {
    ...
    count_triangles_in_chunks_cs = ComputeShader((p / "shaders" / "voxel_rasterization" / "count_triangles_in_chunks.glsl").string()); 
    scan_blocks_cs = ComputeShader((p / "shaders" / "voxel_rasterization" / "scan_blocks.glsl").string());
    add_block_offsets_cs = ComputeShader((p / "shaders" / "voxel_rasterization" / "add_block_offsets.glsl").string());
    fix_last_cs = ComputeShader((p / "shaders" / "voxel_rasterization" / "fix_last.glsl").string());
    copy_offsets_to_cursor_cs = ComputeShader((p / "shaders" / "voxel_rasterization" / "copy_offsets_to_cursor.glsl").string());
    fill_triangle_indices_cs = ComputeShader((p / "shaders" / "voxel_rasterization" / "fill_triangle_indices.glsl").string());
    voxelize_cs = ComputeShader((p / "shaders" / "voxel_rasterization" / "voxelize.glsl").string());
    clear_voxels_cs = ComputeShader((p / "shaders" / "voxel_rasterization" / "clear_voxels.glsl").string()); // ПОЗЖЕ УБРАТЬ!!!
    roi_reduce_indices_cs = ComputeShader((p / "shaders" / "voxel_rasterization" / "roi_reduce_indices.glsl").string());
    roi_reduce_pairs_cs = ComputeShader((p / "shaders" / "voxel_rasterization" / "roi_reduce_pairs.glsl").string());
    build_active_chunks_cs = ComputeShader((p / "shaders" / "voxel_rasterization" / "build_active_chunks.glsl").string());
    roi_finalize_cs = ComputeShader((p / "shaders" / "voxel_rasterization" / "roi_finalize.glsl").string());
    clear_chunks_cs = ComputeShader((p / "shaders" / "voxel_grid" / "clear_chunks.glsl").string()); // ПОЗЖЕ УБРАТЬ!!!
    ...
    mesh_counters_reset_cs = ComputeShader((p / "shaders" / "voxel_grid" / "mesh_counters_reset.glsl").string()); // ПОЗЖЕ УБРАТЬ!!!
    ...
    cmdcount_reset_cs = ComputeShader((p / "shaders" / "voxel_grid" / "cmdcount_reset.glsl").string()); // ПОЗЖЕ УБРАТЬ!!!
    build_indirect_cmds_cs = ComputeShader((p / "shaders" / "voxel_grid" / "build_indirect_cmds.glsl").string());
    reset_dirty_count_cs = ComputeShader((p / "shaders" / "voxel_grid" / "reset_dirty_count.glsl").string()); // ПОЗЖЕ УБРАТЬ!!!
    evict_buckets_reset_cs = ComputeShader((p / "shaders" / "voxel_grid" / "evict_buckets_reset.glsl").string());
    evict_buckets_build_cs = ComputeShader((p / "shaders" / "voxel_grid" / "evict_buckets_build.glsl").string());
    evict_low_priority_cs = ComputeShader((p / "shaders" / "voxel_grid" / "evict_low_priority.glsl").string());
    ...
    reset_load_list_counter_cs = ComputeShader((p / "shaders" / "voxel_grid" / "reset_load_list_counter.glsl").string()); // ПОЗЖЕ УБРАТЬ!!!
