...
void LightingSystem::init(VulkanEngine& engine) {
    this->engine = &engine;
    ...
    light_source_ssbo.create(engine, sizeof(LightSource) * max_num_light_sources);
    lights_in_clusters_ssbo.create(engine, sizeof(unsigned int) * lights_in_clusters_size);
    num_lights_in_clusters_ssbo.create(engine, sizeof(unsigned int) * total_clusters_count);
    cluster_aabbs_ssbo.create(engine, sizeof(AABB) * total_clusters_count);
    ...
    // layout(std430, binding=4) buffer ClusterAABBs { AABB cluster_aabbs[]; };
    ...
    // layout(std430, binding=7) buffer OutIndices { uint out_indices[]; };


    DescriptorSetBundleBuilder builder = DescriptorSetBundleBuilder();
    builder.add_uniform_buffer(0, lighting_system_uniform_buffer, VK_SHADER_STAGE_COMPUTE_BIT);
    builder.add_storage_buffer(4, cluster_aabbs_ssbo, VK_SHADER_STAGE_COMPUTE_BIT);
    builder.add_storage_buffer(5, light_source_ssbo, VK_SHADER_STAGE_COMPUTE_BIT);
    builder.add_storage_buffer(6, num_lights_in_clusters_ssbo, VK_SHADER_STAGE_COMPUTE_BIT);
    builder.add_storage_buffer(7, lights_in_clusters_ssbo, VK_SHADER_STAGE_COMPUTE_BIT);
    descriptor_set_bundle = builder.create(engine.device);

    pipeline.create(engine.device, descriptor_set_bundle, *light_indices_for_clusters_program);
...
void LightingSystem::set_cluster_aabbs(std::vector<AABB>& aabbs) {
    ...
    cluster_aabbs_ssbo.update_data(aabbs.data(), aabbs.size() * sizeof(AABB));
}
...
void LightingSystem::update_cluster_structure(const Camera& camera) {
    ...
    if (cluster_near != camera.near_plane) {
        cluster_near = camera.near_plane;
        need_recreate_clusters = true;
    }
    if (cluster_far != camera.far_plane) {
        cluster_far = camera.far_plane;
        need_recreate_clusters = true;
    }
...
void LightingSystem::update(const Camera& camera) {
    update_cluster_structure(camera);
