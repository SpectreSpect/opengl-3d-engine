...
public:
    ...
    void bind_combined_image_sampler(uint32_t binding, Texture& texture);
...
public:
    struct Entry {
        ...
        std::variant<std::monostate, VideoBuffer*, Texture*> resource;
    };
    ...
    void add_combined_image_sampler(uint32_t binding, Texture& texture, VkShaderStageFlags stage_flags = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT);
