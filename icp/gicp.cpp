...
double GICP::step(PointCloudFrame& source_point_cloud,
                  const PointCloudFrame& target_point_cloud,
                  ...
                  const std::vector<glm::vec4>& target_normals)
{
    ...
    for (size_t i = 0; i < target_points.size(); i++) {
        ...
        glm::vec3 n_world = transform_normal_world(target_point_cloud.point_cloud, n_local);

        target_points_world.push_back(transform_point_world(target_point_cloud.point_cloud, q_local));
        target_normals_world.push_back(n_world);
        target_covs_world.push_back(covariance_from_normal(n_world, gicp_eps));
    }
    ...
    for (size_t i = 0; i < source_points.size(); i++) {
        ...
        // Source point and source normal in world space
        glm::vec3 x = transform_point_world(source_point_cloud.point_cloud, p_local);
        glm::vec3 n_src_world = transform_normal_world(source_point_cloud.point_cloud, n_src_local);
...
double GICP::step_test(PointCloudFrame& source_point_cloud,
                  const PointCloudFrame& target_point_cloud,
                  const std::vector<glm::vec4>& source_normals,
                  const std::vector<glm::vec4>& target_normals)
{
...
// double GICP::align_gpu(PointCloudFrame& source_point_cloud,
//                         const PointCloudFrame& target_point_cloud,
//                         const std::vector<glm::vec3>& source_normals,
//                         const std::vector<glm::vec3>& target_normals) {
