...
void main() {
    vec3 light_pos = vec3(-20, 4, 0);
    // vec3 light_dir = normalize(vec3(1, 1.5, 1.3));
    vec3 light_dir = normalize(light_pos - vFragPos);
    ...
    // Specular
    float distance_to_light = distance(light_pos, vFragPos);
    float light_radius = 20.0;
    float light_intensity = max((light_radius - distance_to_light) / light_radius, 0.0);
    ...
    vec3 color = specular * light_intensity * vec3(0.0, 0.0, 1.0) + diffuse * light_intensity * vec3(0, 0, 1.0);
